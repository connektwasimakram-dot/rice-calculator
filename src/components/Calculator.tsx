...
export default function Calculator() {
  ...
  const handleExportPDF = () => {
    ...
    const rows = [
      ...
      ["18kg Bag Landing Price", `${landedCost18KgAed.toFixed(2)} AED`],
      ["Landed Cost / Metric Tonne", `${landedCostPerMtAed.toFixed(2)} AED`],
    ...
    pdf.rect(margin, 580, pageWidth - margin * 2, 26, "F");

    pdf.setTextColor(5, 150, 105); // emerald-600
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(10);
    pdf.text("Net Profit / Container (AED)", margin + 8, 596);
    pdf.text(`${netProfitAed.toFixed(2)} AED`, pageWidth - margin - 8, 596, { align: "right" });
              ...
              <div className="lg:col-span-7 space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-lg border border-slate-200 p-2.5 bg-slate-50/50">
                      ...
                      <input
                        ...
                        className="mt-1 block w-full rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-xs sm:text-sm font-mono text-slate-900 shadow-2xs focus:border-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-900"
                      />
                    </div>
                    <div className="rounded-lg border border-slate-200 p-2.5 bg-slate-50/50">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="rounded-lg border border-slate-200 p-2.5 bg-slate-50/50">
                      <label className="block text-xs font-medium text-slate-600">Cargo (MT)</label>
                      <input
                        type="number"
                        name="cargoMt"
                        ...
                        className="mt-1 block w-full rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-xs sm:text-sm font-mono text-slate-900 shadow-2xs focus:border-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-900"
                      />
                      <span className="mt-0.5 block text-[10px] text-slate-400 font-mono tabular-nums">{totalBags.toFixed(0)} bags</span>
                    </div>
                    <div className="rounded-lg border border-slate-200 p-2.5 bg-slate-50/50">
                      <label className="block text-xs font-medium text-slate-600">Bag Weight (kg)</label>
                      <input
                        type="number"
                        name="bagSizeKg"
                        ...
                        className="mt-1 block w-full rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-xs sm:text-sm font-mono text-slate-900 shadow-2xs focus:border-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-900"
                      />
                      <span className="mt-0.5 block text-[10px] text-slate-400 font-mono">Typically 50kg</span>
                    </div>
                    <div className="rounded-lg border border-slate-200 p-2.5 bg-slate-50/50">
                      <label className="block text-xs font-medium text-slate-600">Ex-Mill (INR / kg)</label>
                      <input
                        type="number"
                        name="exMillPrice"
                        ...
                        className="mt-1 block w-full rounded-md border border-slate-300 bg-white px-2.5 py-1.5 text-xs sm:text-sm font-mono text-slate-900 shadow-2xs focus:border-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-900"
                      />
                      <span className="mt-0.5 block text-[10px] text-slate-400 font-mono">Clean milled</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-lg border border-slate-200 p-2.5 bg-slate-50/50">
                      <label className="block text-xs font-medium text-slate-600">Packaging (INR / bag)</label>
                      <input
                        type="number"
                        name="packaging"
                    ...
                    <div className="rounded-lg border border-slate-200 p-2.5 bg-slate-50/50">
                      <label className="block text-xs font-medium text-slate-600">Inland Logistics & CFS (INR)</label>
                      <input
                        type="number"
                ...
                <div className="space-y-2">
                  <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                    ...
                    4. Maritime Freight & Port Handling
                  </span>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-lg border border-slate-200 p-2.5 bg-slate-50/50">
                      <label className="block text-xs font-medium text-slate-600">Ocean Freight (USD)</label>
                      <input
                        type="number"
                    ...
                    <div className="rounded-lg border border-slate-200 p-2.5 bg-slate-50/50">
                      <label className="block text-xs font-medium text-slate-600">UAE Port & Customs (AED)</label>
                      <input
                        type="number"
                        name="uaePortAed"
                ...
                <div className="rounded-lg border border-emerald-300/80 bg-emerald-50/30 p-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] sm:text-[11px] font-medium text-emerald-700">AED per 50kg bag</span>
                  </div>
                  <div className="mt-1.5 flex items-center gap-2">
                    <input
                      ...
                      onChange={handleInputChange}
                      className="block w-full rounded-md border border-emerald-400 bg-white px-2.5 py-1.5 text-sm sm:text-base font-semibold font-mono text-slate-900 shadow-2xs focus:border-emerald-600 focus:outline-none focus:ring-1 focus:ring-emerald-600"
                    />
                    <span className="text-xs font-semibold text-slate-700 font-mono whitespace-nowrap">AED / bag</span>
                  </div>
                </div>
              </div>

              {/* Financial Summary & KPIs: 5 Columns */}
              <div className="lg:col-span-5 flex flex-col justify-between rounded-xl border border-slate-200 bg-slate-50/60 p-4 sm:p-5">
                <div>
                  <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                    <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-900">
                      Commercial P&L Breakdown
                    </h3>
                    <span className="text-[10px] font-mono text-slate-500">Live Estimates</span>
                  </div>

                  {/* Primary Metric Card */}
                  <div className="mt-3.5 rounded-lg bg-slate-900 p-4 text-white shadow-xs">
                    <span className="block text-[11px] font-medium uppercase tracking-wider text-slate-400">
                      Net Profit / Container
                    </span>
                    <div className="mt-0.5 flex items-baseline gap-2">
                      <span className="text-2xl sm:text-3xl font-semibold tracking-tight font-mono tabular-nums text-white">
                        {netProfitAed.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                      <span className="text-xs sm:text-sm font-medium text-emerald-400">AED</span>
                    </div>

                    <div className="mt-2.5 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-slate-300 pt-2 border-t border-slate-800 font-mono tabular-nums">
                  ...
                  {/* Detailed Unit Cost Table - tightened */}
                  <div className="mt-3.5 space-y-1.5 text-xs">
                    ...
                    <div className="flex justify-between items-center py-1 border-b border-slate-200/80 bg-white/70 px-2 rounded-sm">
                ...
                <div className="mt-4 space-y-2 pt-3 border-t border-slate-200">
                  <div className="grid grid-cols-2 gap-2.5">
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-1.5 rounded-md bg-slate-900 px-3.5 py-2 text-xs font-semibold text-white shadow-xs transition hover:bg-slate-800 active:scale-[0.99]"
                    >
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-1.5 rounded-md bg-emerald-600 px-3.5 py-2 text-xs font-semibold text-white shadow-xs transition hover:bg-emerald-500 active:scale-[0.99]"
                    >
